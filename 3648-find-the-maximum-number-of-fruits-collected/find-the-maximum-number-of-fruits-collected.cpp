...
public:
    int maxCollectedFruits(vector<vector<int>>& fruits) {
        ...
        ///child1collect - diagonal element 
        int result=0;
        for(int i=0;i<n;i++){
            result += fruits[i][i];
        }
        ////before child 2 and child 3 nullify all the cells that can't be visited by 2 and 3
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                if(i<j && j+i < n-1){
                    t[i][j] = 0;
